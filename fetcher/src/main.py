...
def fetch_async():
    ...
    выполнение нельзя назвать параллельным, так как ОС
    постоянно переключает контекст между потоками. Затем, я
    дожидаюсь всех ответов от сервера и отправляю в ответе
